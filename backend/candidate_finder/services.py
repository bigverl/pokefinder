"""
Make suggestions to resolve the following things:
- Inconsistency of use of default dict vs normal dict
- Not ultra-specific return types for pokemon_repository.py get_x_indexes vs the individual ones in the services/get_x.py originals
- Where do you put these global variables -> STAT_WEIGHT_WHATEVER
...
class CandidateFinder():
    ...
    def __init__(self, repository: PokemonRepository):
        ...
        self._species_index = self.repository.get_species_index()
        if not self._species_index:
            logger.error("Empty species index")
            raise ValueError("Repository returned empty SPECES index")
    ...
    def get_pokemon_by_move(
        ...
        filtered = {
            ...
            if (include_legendary or not self._species_index[name]["is_legendary"])
            and (include_mythical or not self._species_index[name]["is_mythical"])
            and (include_ultra_beasts or not self._species_index[name]["is_ultra_beast"])
        }
    ...
    def get_pokemon_by_stats(
        ...
        filtered_by_species = {
            ...
            and (include_legendary or not self._species_index[name]["is_legendary"])
            and (include_mythical or not self._species_index[name]["is_mythical"])
            and (include_ultra_beasts or not self._species_index[name]["is_ultra_beast"])
        }
