...
def fake_repo():
    """Real SQLAlchemyRepository with fake data injected. No DB needed."""
...
@pytest.fixture(scope="session")
def finder(fake_repo) -> CandidateFinderService:
    """CandidateFinderService with fake repository (no DB)"""
    return CandidateFinderService(fake_repo)


@pytest.fixture(scope="session")
def coverage_analyzer(fake_repo) -> CoverageAnalyzerService:
    """CoverageAnalyzerService with fake repository (no DB)"""
